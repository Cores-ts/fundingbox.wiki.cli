...
class Start extends command_1.Command {
    async run() {
        ...
        const WORKING_FOLDER = path_1.join(PWD, (_b = userConfig.localWikiFolder) !== null && _b !== void 0 ? _b : '', userConfig.localWikiFolder ? 'fundingbox-wiki' : '');
    ...
    getGitRootDirectory(directory) {
        while (!fs_1.existsSync(path_1.join(directory, '.git'))) {
